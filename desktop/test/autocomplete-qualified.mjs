...
function testQualifiedColumnCandidates() {
  ...
  const withFrom = collectAtCursor(autocomplete, 'SELECT budget.na FROM budget', 'SELECT budget.na'.length);
  ...
  const afterDot = collectAtCursor(autocomplete, 'SELECT budget. FROM budget', 'SELECT budget.'.length);
