...
- CSV 使用原生另存为对话框；
- 记忆窗口位置和尺寸；
- 支持轻量启动：只运行 Native Core、托盘和后台任务，不创建 WebView2；
- 关闭主窗口时保存状态并销毁 WebView，托盘左键按需打开桌面端，右键菜单仅提供打开主界面、无勾选态的进入轻量模式和退出；
...
- 主窗口位置和尺寸：同目录的 Tauri window-state 文件；
...
- 轻量模式验收：冷启动无 app-owned WebView2、托盘重建唯一主窗口、关闭后 WebView2 自然清零、托盘退出结束原生进程；不能强杀 WebView2 制造通过结果。
