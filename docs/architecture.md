...
没有顶层 `LIMIT`、`OFFSET` 或 `FETCH` 的普通 SELECT 和只读查询型 CTE 会自动追加 `LIMIT/OFFSET`，默认每页 100 条，并通过带 `LIMIT 1000` 的派生表 COUNT 计算分页范围。PG 表数据浏览的页大小交给 Archery 的 `limit_num` 追加，避免重复 `LIMIT`；所有响应仍不能超过 1000 条。每个多 SQL 结果页签独立保存执行时的实例、数据库、schema、页码和页大小；翻页只重查当前结果，并同步刷新 COUNT。数据修改 CTE、锁定查询、`SELECT INTO`、DML、DDL 以及用户自行分页的 SQL 保持单次执行，但响应仍不能超过 1000 条。
