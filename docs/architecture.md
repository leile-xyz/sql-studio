...
- `paged-export.mjs`、`export-service.mjs`、`csv-export-actions.mjs`：逐页结果校验与双端全量导出编排；
...
没有顶层 `LIMIT`、`OFFSET` 或 `FETCH` 的普通 SELECT 和只读查询型 CTE 会自动追加 `LIMIT/OFFSET`，默认每页 1000 条，并通过派生表 COUNT 查询精确总数。每个多 SQL 结果页签独立保存执行时的实例、数据库、schema、页码和页大小；翻页只重查当前结果，并同步刷新 COUNT。数据修改 CTE、锁定查询、`SELECT INTO`、DML、DDL 以及用户自行分页的 SQL 保持单次执行，避免自动重查产生副作用。
