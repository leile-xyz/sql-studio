...
`main` 分支维护 Windows 桌面端；浏览器扩展实现位于 `extension` 分支。

## 技术栈

桌面端基于 Tauri 2、Rust 和 WebView2，适合 Windows 内网环境直连 Archery。网络与 Django 会话由 Rust 宿主处理，密码保存到 Windows 凭据管理器，CSV 通过原生另存为对话框导出。
...
- 表数据精确总数与分页（每页最高 1000 条）、组合排序、WHERE 条件片段、结构查看和全量 CSV 导出；
...
- 支持原生轻量模式：后台只保留 Rust 进程、托盘、配置状态和后台任务，不创建 WebView；关闭主窗口会保存界面状态并销毁 WebView，左键托盘按需打开桌面端，右键菜单可进入轻量模式或彻底退出；
- 控制台普通 SELECT 默认每页 100 条并展示精确总数，支持翻页、全量 CSV 和查询历史；
