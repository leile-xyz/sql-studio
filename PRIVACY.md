...
SQL Studio 是连接用户自有 Archery/dbadmin 服务的本地客户端。项目本身不提供云服务，不包含遥测、广告或第三方分析代码。
...
- 登录用户名和可选的“记住密码”数据；
- 查询历史、控制台草稿和界面状态；
- 从用户配置的 Archery/dbadmin 返回的数据库、表结构和查询结果。
...
本说明不覆盖用户自行部署的 Archery/dbadmin 服务；其数据处理方式由对应服务的运营者决定。
