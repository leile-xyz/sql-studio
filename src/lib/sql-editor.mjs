...
const TABLE_ALIAS_EXCLUDED_WORDS = new Set([
  'as', 'where', 'join', 'left', 'right', 'inner', 'outer', 'full', 'cross', 'natural', 'on', 'using',
...
export function resolveSqlAutocompleteInput(source) {
  ...
  if (qualified) {
    return Object.freeze({
      qualifier: qualified[1] || qualified[2] || qualified[3],
      prefix: qualified[4] || '',
    });
  }
...
export class SqlAutocomplete {
  ...
  collectQualifiedColumnItems(options) {
    ...
    )) || references.find(candidate => candidate.table.toLowerCase() === lowerQualifier);
  ...
  collectColumnItems(options) {
    ...
    for (const reference of this.referencedTableRefs(sql)) {
      ...
      const tableKey = schema + '\u001f' + reference.table;
